...
public interface AsoControleRepository  extends JpaRepository<Asocontrole, Integer>{
	...
	@Query("select a from Asocontrole a where a.finalizado=0 and  a.datavencimento<= :data and a.funcionario.situacao='Ativo' and a.asotipo<>5 order by a.datavencimento")
	...
	//Data Vencimento e funcao
		...
		//Baixa anterior 
		@Query("select a from Asocontrole a where a.asotipo.tipo= :tipo " 
				+ " and a.finalizado=0 and a.funcionario.idfuncionario= :idfuncionario ")
		Optional<Asocontrole> findLast(
		@Param("idfuncionario") int idfuncionario, @Param("tipo") String tipo);	
